...
├── index.html            页面骨架（菜单 + 做法详情面板 + 设置面板）
├── styles.css            样式（移动端优先）
├── app.js                全部前端逻辑：设置读写 + 随机菜单 + 屏蔽过滤 + 做法详情
├── data/
...
> **状态：未部署。** 本机已安装 CloudBase CLI 2.12.2，但**没有登录态、也没有环境 ID**，
> 因此 **CloudBase 上没有任何真实线上地址**。
>
> 尝试过 `tcb login`：CLI 会拉起浏览器授权页并等待人工确认，等待 240 秒超时，未取得登录态。
> 这一步**没法由脚本代替完成**——它需要账号持有人在浏览器里确认授权。
>
> 项目侧的静态适配（零构建、纯静态资源、干净部署目录）已经完成并验证：
> `node tools/deploy-cloudbase.mjs` 能产出一个只含 6 个站点文件、共约 750 KB 的 `dist/`。
