...
选好菜之后，还能填一个主食克数，顺手看到**这顿饭大概多少热量、人均多少**——
...
├── index.html            页面骨架（菜单 + 主食 + 营养 + 做法详情面板 + 设置面板）
├── styles.css            样式（移动端优先）
├── app.js                界面逻辑：设置读写 + 随机菜单 + 屏蔽过滤 + 渲染
...
保存的内容：`mealSize`（每餐道数）、`householdSize`（用餐人数）、`blockedKeywords`（屏蔽词）、
`healthProfile`（健康资料）、`staple`（主食种类与克数）。
...
人数只影响**主食人均量、整顿饭总营养和人均营养**，不参与 BMI / BMR——那两项只跟个人资料有关。
和「每餐菜品数量」完全独立：`2 人 + 2 道`、`5 人 + 4 道`、`8 人 + 8 道` 都合法。
...
主页菜单下方可以选主食（10 种：米饭、面条、馒头、面包、玉米、红薯、燕麦、小米、糙米饭、大米），
并填一个克数。

**克数是「本餐全家实际吃掉的量」，不是每个人的量。** 填 750g、5 人，人均就是 150g。
默认按「人数 × 150g」自动填，手动改过之后就不再自动覆盖。

### 健康参考

设置里可以填性别、年龄、身高、体重、活动水平，给出下面这些**粗略参考值**：
...
资料没填全时不出结果，也不猜。这一节展示的是**参考范围**，不是「你必须吃多少」。
...
- **设置**：用餐人数 1–8、每餐 2/4/6/8 道、屏蔽关键词、健康参考，存在浏览器本地，刷新不丢
- **主食**选择与克数，**本餐营养估算**（整桌 + 人均），**单道菜营养估算**（含覆盖率）
