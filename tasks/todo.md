...
- [x] **T16** **用户真机验证** —— **已完成**（真机扫码打开、转发入口出现、好友扫码使用，均已在真机实测）
...
- [x] 真机预览二维码 —— 当时受微信服务端 AppID 限制未能完成；
      **后续已解决**：接入正式 AppID `wxa20de818e7518885` 后，真机预览与体验版二维码均已打通
...
- [x] 主体类型、认证费用：**未查到**（前者被"请先补充小程序基本信息"拦住，
      后者只有走到第⑤步才显示）。**已因「不做认证」决策关闭**，不再需要查证
...
- [x] **真机验证通过**（2026-09-24）：好友扫码**成功打开并使用**小程序——
      印证「**项目成员即可打开体验版，无需另加体验成员**」。
      注意：项目成员权限远大于体验（**可登录小程序管理后台**），
      仅想让人试玩、不愿给后台权限时，仍应走体验成员
- [x] README 已回填该结论
