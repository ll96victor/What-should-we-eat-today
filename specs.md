...
- **平台前提（2026-09-24 真机实测后补）**：「能不能转发」是微信的**账号级**能力，
  ...
  平台拒绝。**该前提与体验成员人数无关**（体验成员决定的是"谁能打开体验版"）。

> **口径冲突（待用户决策）**：上面把「转发给朋友」划入 V2，下面又排除
> 「正式发布 / 搜索上线 / 备案 / 商业化」。而转发所依赖的微信认证正落在这条排除线内。
> 即：**在不做认证的前提下，转发功能在代码层可用、在平台层不可用**。
> 保留两条事实并列，是否放开认证由用户决定，不擅自改动需求范围。
